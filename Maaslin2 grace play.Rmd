...
grace_play_15_c_fam = Maaslin2(
    input_data = mouse_5_family, 
    input_metadata = Weight_meta_PGH_b, 
    output = "Weight_family_PGH_b", 
    fixed_effects = c("Weight_g", "Day_Num"),
    random_effects = c("Mouse_ID"))  
...
grace_play_15_pl_weight_genus = Maaslin2(
    input_data = mouse_6_genus, 
    input_metadata = Weight_meta_PL_b, 
    output = "Weight_genus_PL", 
    fixed_effects = c("Weight_g"),
    random_effects = c("Mouse_ID"))

grace_play_15_pl_weight_genus_b = Maaslin2(
    input_data = mouse_6_genus, 
    input_metadata = Weight_meta_PL_b, 
    output = "Weight_genus_PL_b", 
    fixed_effects = c("Weight_g", "Day_Num"),
    random_effects = c("Mouse_ID")) # can I get heatmap? Yes but none of them correlate with each other
...
grace_play_15_PGH_long_p = Maaslin2(
    input_data = mouse_2_phylum, 
    input_metadata = Weight_meta_PGH_b, 
    output = "PGH_phylum_longitduinally", 
    fixed_effects = c("Day_Num"),
    random_effects = c("Mouse_ID")) 

grace_play_15_PGH_long_c = Maaslin2(
    input_data = mouse_3_class, 
    input_metadata = Weight_meta_PGH_b, 
    output = "PGH_class_longitduinally", 
    fixed_effects = c("Day_Num"),
    random_effects = c("Mouse_ID")) 
...
grace_play_15_PL_long_p = Maaslin2(
    input_data = mouse_2_phylum, 
    input_metadata = Weight_meta_PL_b, 
    output = "PL_phylum_longitduinally", 
    fixed_effects = c("Day_Num"),
    random_effects = c("Mouse_ID")) 

grace_play_15_PL_long_c = Maaslin2(
    input_data = mouse_3_class, 
    input_metadata = Weight_meta_PL_b, 
    output = "PL_class_longitduinally", 
    fixed_effects = c("Day_Num"),
    random_effects = c("Mouse_ID")) 
...
grace_play_15_Saline_long_p = Maaslin2(
    input_data = mouse_2_phylum, 
    input_metadata = Weight_meta_Saline_b, 
    output = "Saline_phylum_longitduinally", 
    fixed_effects = c("Day_Num"),
    random_effects = c("Mouse_ID")) # no changes

grace_play_15_Saline_long_c = Maaslin2(
    input_data = mouse_3_class, 
    input_metadata = Weight_meta_Saline_b, 
    output = "Saline_class_longitduinally", 
    fixed_effects = c("Day_Num"),
    random_effects = c("Mouse_ID")) # one minor change


# What about Saline vs PGH at Day 5?
salinevspgh_meta <- df_mouse_metadata %>% 
  filter(Hormone != "PL" & Day == "D8" & Antibiotics == "Non_ABX")

grace_play_16_salinevspgh_genus = Maaslin2(
    input_data = mouse_6_genus, 
    input_metadata = salinevspgh_meta, 
    output = "salinevspgh_genus_d8", 
    fixed_effects = c("Hormone")) # d8 has differences but do we trust that? Stick do d5 i say

# Day 8 version
grace_play_16_salinevspgh_fam = Maaslin2(
    input_data = mouse_5_family, 
    input_metadata = salinevspgh_meta, 
    output = "salinevspgh_family_longitduinally", 
    fixed_effects = c("Hormone")) # still don't trust it... could be contam? but why just in PGH?
```

Seeing if I can look at changes in body composition in mice ("responders vs non-responders" and if that might correlate with either baseline or endpoint characteristics of their gut microbiome?)
``` {r body comp changes1 }
# Trying to make the change data frame from my old R doc
baseline_data <- EchoMRI[EchoMRI$Timepoint == "Baseline", ]
endpoint_data <- EchoMRI[EchoMRI$Timepoint == "Endpoint", ]

# Merge the baseline and endpoint data by Mouse to ensure matching rows
merged_data <- merge(baseline_data, endpoint_data, by = "Mouse", suffixes = c("_baseline", "_endpoint"))
View(merged_data)
# Calculate the absolute and relative percent changes for each metric
change_in_body_comp <- data.frame(
  Mouse = merged_data$Mouse,
  
  # Absolute percent changes
  Abs_Change_Fat = merged_data$Fat_percentage_endpoint - merged_data$Fat_percentage_baseline,
  Abs_Change_Lean_Mass = merged_data$Lean_mass_percentage_endpoint - merged_data$Lean_mass_percentage_baseline,
  Abs_Change_Free_Water = merged_data$Free_water_percentage_endpoint - merged_data$Free_water_percentage_baseline,
  Abs_Change_Total_Water = merged_data$Total_water_percentage_endpoint - merged_data$Total_water_percentage_baseline,
  
  # Relative percent changes
  Rel_Change_Fat = (merged_data$Fat_percentage_endpoint - merged_data$Fat_percentage_baseline) / merged_data$Fat_percentage_baseline * 100,
  Rel_Change_Lean_Mass = (merged_data$Lean_mass_percentage_endpoint - merged_data$Lean_mass_percentage_baseline) / merged_data$Lean_mass_percentage_baseline * 100,
  Rel_Change_Free_Water = (merged_data$Free_water_percentage_endpoint - merged_data$Free_water_percentage_baseline) / merged_data$Free_water_percentage_baseline * 100,
  Rel_Change_Total_Water = (merged_data$Total_water_percentage_endpoint - merged_data$Total_water_percentage_baseline) / merged_data$Total_water_percentage_baseline * 100
)

# check it out
View(change_in_body_comp)
change_in_body_comp <-  change_in_body_comp %>% 
  rename(Mouse_Num = Mouse)

# Ok cool now gotta merge this with meta data
df_mouse_metadata_dmin3 <- df_mouse_metadata %>% 
  filter(Day == "D-3") %>% 
  rownames_to_column(var = "Sample_ID") 
df_mouse_metadata_d0 <- df_mouse_metadata %>% 
  filter(Day == "D0")%>% 
  rownames_to_column(var = "Sample_ID") 
df_mouse_metadata_d5 <- df_mouse_metadata %>% 
  filter(Day == "D5")%>% 
  rownames_to_column(var = "Sample_ID") 

df_mouse_metadata_dmin3 <- df_mouse_metadata_dmin3 %>% 
  rownames_to_column(var = "Sample_ID") 

# making the meta 
# d-3
Perc_change_meta_dmin3 <- left_join(df_mouse_metadata_dmin3, change_in_body_comp, by = "Mouse_Num") %>% 
  column_to_rownames(var = "Sample_ID")
  # filtering to exclude abx
    Perc_change_meta_dmin3 <- Perc_change_meta_dmin3 %>% 
      filter(Antibiotics == "Non_ABX")

# d0
Perc_change_meta_d0 <- left_join(df_mouse_metadata_d0, change_in_body_comp, by = "Mouse_Num") %>% 
  column_to_rownames(var = "Sample_ID")
  # filtering to exclude abx
    Perc_change_meta_d0 <- Perc_change_meta_d0 %>% 
      filter(Antibiotics == "Non_ABX")

# d5
Perc_change_meta_d5 <- left_join(df_mouse_metadata_d5, change_in_body_comp, by = "Mouse_Num") %>% 
  column_to_rownames(var = "Sample_ID")
  # filtering to exclude abx
    Perc_change_meta_d5 <- Perc_change_meta_d5 %>% 
      filter(Antibiotics == "Non_ABX")

# META DONE!
    
# Q1: Kind of a dumb question, but not considering hormone group, do baseline levels of any microbes correlate with how much fat or lean mass that mouse eventually changed by?
    
    # fat
    # genus level
    grace_play_17_Baseline_microbes_to_fat_change_genus = Maaslin2(
    input_data = mouse_6_genus, 
    input_metadata = Perc_change_meta_dmin3, 
    output = "Baseline_microbes_to_fat_change_genus", 
    fixed_effects = c("Abs_Change_Fat")) # no lol
    
    # phylum level
    grace_play_17_Baseline_microbes_to_fat_change_phylum = Maaslin2(
    input_data = mouse_2_phylum, 
    input_metadata = Perc_change_meta_dmin3, 
    output = "Baseline_microbes_to_fat_change_phylum", 
    fixed_effects = c("Abs_Change_Fat")) # yes tenericutes
    
    grace_play_17_Baseline_microbes_to_fat_change_phylum = Maaslin2(
    input_data = mouse_2_phylum, 
    input_metadata = Perc_change_meta_dmin3, 
    output = "Baseline_microbes_to_fat_change_phylum_controlledforhormone", 
    fixed_effects = c("Abs_Change_Fat"),
    random_effects = c("Hormone")) # but not when u control for hormone 
    
    # class level
    grace_play_17_Baseline_microbes_to_fat_change_class = Maaslin2(
    input_data = mouse_3_class, 
    input_metadata = Perc_change_meta_dmin3, 
    output = "Baseline_microbes_to_fat_change_class", 
    fixed_effects = c("Abs_Change_Fat")) # no -- deleting file
    
    # order level
    grace_play_17_Baseline_microbes_to_fat_change_order = Maaslin2(
    input_data = mouse_4_order, 
    input_metadata = Perc_change_meta_dmin3, 
    output = "Baseline_microbes_to_fat_change_order", 
    fixed_effects = c("Abs_Change_Fat")) # no -- deleting file
    
    # family level
    grace_play_17_Baseline_microbes_to_fat_change_family = Maaslin2(
    input_data = mouse_5_family, 
    input_metadata = Perc_change_meta_dmin3, 
    output = "Baseline_microbes_to_fat_change_family", 
    fixed_effects = c("Abs_Change_Fat")) # no-- deleting file
    
    # lean mass
    # genus level
    grace_play_17_Baseline_microbes_to_lean_change_genus = Maaslin2(
    input_data = mouse_6_genus, 
    input_metadata = Perc_change_meta_dmin3, 
    output = "Baseline_microbes_to_lean_change_genus", 
    fixed_effects = c("Abs_Change_Lean_Mass")) # oh yes lots
    
    # hormone as random effect
     grace_play_17_Baseline_microbes_to_lean_change_genus = Maaslin2(
    input_data = mouse_6_genus, 
    input_metadata = Perc_change_meta_dmin3, 
    output = "Baseline_microbes_to_lean_change_genus_controlledforhormone", 
    fixed_effects = c("Abs_Change_Lean_Mass"),
    random_effects = c("Hormone")) # now none
    
    # phylum level
    grace_play_17_Baseline_microbes_to_lean_change_phylum = Maaslin2(
    input_data = mouse_2_phylum, 
    input_metadata = Perc_change_meta_dmin3, 
    output = "Baseline_microbes_to_lean_change_phylum_controlledforhormone", 
    fixed_effects = c("Abs_Change_Lean_Mass"),
    random_effects = c("Hormone")) # now none 
    
    # but this is baseline microbiome compositions...? 

```

Ok now this is low power because its n = 6 per group, but can try looking at within-group changes
3 groups x 3 time points x 4 metrics = 36 analyses. will start with just genus, order, and phylum? so 108 total...
``` {r body comp changes 2}
Perc_change_meta_d5_PGH <- Perc_change_meta_d5 %>% 
  filter(Hormone == "PGH")

# PGH D5 
    # LEAN
PGH_d5_lean_g = Maaslin2(
    input_data = mouse_6_genus, 
    input_metadata = Perc_change_meta_d5_PGH, 
    output = "PGH_d5_lean_g", 
    fixed_effects = c("Abs_Change_Lean_Mass")) # boring deleted file

# try fam and order? 
PGH_d5_lean_f = Maaslin2(
    input_data = mouse_5_family, 
    input_metadata = Perc_change_meta_d5_PGH, 
    output = "PGH_d5_lean_f", 
    fixed_effects = c("Abs_Change_Lean_Mass")) # nothin. delete. Lactobacillaceae close tho (coef 1.5, unadjusted p = 0.01)

PGH_d5_lean_0 = Maaslin2(
    input_data = mouse_4_order, 
    input_metadata = Perc_change_meta_d5_PGH, 
    output = "PGH_d5_lean_o", 
    fixed_effects = c("Abs_Change_Lean_Mass")) # nothin. delete. Lactobacillales close tho (coef 1.5, unadjusted p = 0.01)


PGH_d5_lean_p = Maaslin2(
    input_data = mouse_2_phylum, 
    input_metadata = Perc_change_meta_d5_PGH, 
    output = "PGH_d5_lean_p", 
    fixed_effects = c("Abs_Change_Lean_Mass")) # boring deleted file
    
    # FAT -- dont test others. bad p vals
PGH_d5_lean_g = Maaslin2(
    input_data = mouse_6_genus, 
    input_metadata = Perc_change_meta_d5_PGH, 
    output = "PGH_d5_fat_g", 
    fixed_effects = c("Abs_Change_Fat")) # boring deleted file
...
PGH_d5_lean_p = Maaslin2(
    ...
    # TOTAL WATER
PGH_d5_twater_g = Maaslin2(
    input_data = mouse_6_genus, 
    input_metadata = Perc_change_meta_d5_PGH, 
    output = "PGH_d5_twater_g", 
    fixed_effects = c("Abs_Change_Total_Water"))

# try fam and order? 
PGH_d5_twater_f = Maaslin2(
    input_data = mouse_5_family, 
    input_metadata = Perc_change_meta_d5_PGH, 
    output = "PGH_d5_twater_f", 
    fixed_effects = c("Abs_Change_Total_Water")) # nothin. delete. Lactobacillaceae closest tho (coef 1.3, unadjusted p = 0.05)

PGH_d5_twater_o = Maaslin2(
    input_data = mouse_4_order, 
    input_metadata = Perc_change_meta_d5_PGH, 
    output = "PGH_d5_twater_o", 
    fixed_effects = c("Abs_Change_Total_Water")) # nothin. delete. 

PGH_d5_twater_p = Maaslin2(
    input_data = mouse_2_phylum, 
    input_metadata = Perc_change_meta_d5_PGH, 
    output = "PGH_d5_twater_p", 
    fixed_effects = c("Abs_Change_Total_Water")) # boring deleted file

    # FREE WATER
PGH_d5_fwater_g = Maaslin2(
    input_data = mouse_6_genus, 
    input_metadata = Perc_change_meta_d5_PGH, 
    output = "PGH_d5_fwater_g", 
    fixed_effects = c("Abs_Change_Free_Water"))  # boring deleted file

PGH_d5_fwater_p = Maaslin2(
    input_data = mouse_2_phylum, 
    input_metadata = Perc_change_meta_d5_PGH, 
    output = "PGH_d5_fwater_p", 
    fixed_effects = c("Abs_Change_Free_Water")) # boring deleted file

# PL D5
Perc_change_meta_d5_PL <- Perc_change_meta_d5 %>% 
  filter(Hormone == "PL")
    # LEAN
PL_d5_lean_g = Maaslin2(
    input_data = mouse_6_genus, 
    input_metadata = Perc_change_meta_d5_PL, 
    output = "PL_d5_lean_g", 
    fixed_effects = c("Abs_Change_Lean_Mass")) # boring deleted file

PL_d5_lean_p = Maaslin2(
    input_data = mouse_2_phylum, 
    input_metadata = Perc_change_meta_d5_PL, 
    output = "PL_d5_lean_p", 
    fixed_effects = c("Abs_Change_Lean_Mass")) # boring deleted file
...
PL_d5_lean_p = Maaslin2(
    ...
    # TOTAL WATER
PL_d5_twater_g = Maaslin2(
    input_data = mouse_6_genus, 
    input_metadata = Perc_change_meta_d5_PL, 
    output = "PL_d5_twater_g", 
    fixed_effects = c("Abs_Change_Total_Water")) # boring deleted file

PL_d5_twater_p = Maaslin2(
    input_data = mouse_2_phylum, 
    input_metadata = Perc_change_meta_d5_PL, 
    output = "PL_d5_twater_p", 
    fixed_effects = c("Abs_Change_Total_Water")) # boring deleted file

    # FREE WATER
PL_d5_fwater_g = Maaslin2(
    input_data = mouse_6_genus, 
    input_metadata = Perc_change_meta_d5_PL, 
    output = "PL_d5_fwater_g", 
    fixed_effects = c("Abs_Change_Free_Water")) # stuff! the ones that lost more free water from baseline to endpoint also had higher endpoint levels of this:
# k__Bacteria.p__Firmicutes.c__Clostridia.o__Clostridiales.f__Veillonellaceae.g__Dialister ok???

# fam
PL_d5_fwater_f = Maaslin2(
    input_data = mouse_5_family, 
    input_metadata = Perc_change_meta_d5_PL, 
    output = "PL_d5_fwater_f", 
    fixed_effects = c("Abs_Change_Free_Water")) # same thing: fam Veillonellaceae

# order
PL_d5_fwater_o = Maaslin2(
    input_data = mouse_4_order, 
    input_metadata = Perc_change_meta_d5_PL, 
    output = "PL_d5_fwater_o", 
    fixed_effects = c("Abs_Change_Free_Water")) # boring deleted file

PL_d5_fwater_p = Maaslin2(
    input_data = mouse_2_phylum, 
    input_metadata = Perc_change_meta_d5_PL, 
    output = "PL_d5_fwater_p", 
    fixed_effects = c("Abs_Change_Free_Water")) # boring deleted file

# Saline D5
Perc_change_meta_d5_Saline <- Perc_change_meta_d5 %>% 
  filter(Hormone == "Saline")
    # LEAN
Saline_d5_lean_g = Maaslin2(
    input_data = mouse_6_genus, 
    input_metadata = Perc_change_meta_d5_Saline, 
    output = "Saline_d5_lean_g", 
    fixed_effects = c("Abs_Change_Lean_Mass")) # boring deleted file

Saline_d5_lean_p = Maaslin2(
    input_data = mouse_2_phylum, 
    input_metadata = Perc_change_meta_d5_Saline, 
    output = "Saline_d5_lean_p", 
    fixed_effects = c("Abs_Change_Lean_Mass")) # boring deleted file
...
Saline_d5_lean_g = Maaslin2(
    input_data = mouse_6_genus, 
    input_metadata = Perc_change_meta_d5_Saline, 
    output = "Saline_d5_fat_g", 
    fixed_effects = c("Abs_Change_Fat")) # boring deleted file

Saline_d5_lean_p = Maaslin2(
    input_data = mouse_2_phylum, 
    input_metadata = Perc_change_meta_d5_Saline, 
    output = "Saline_d5_fat_p", 
    fixed_effects = c("Abs_Change_Fat")) # boring deleted file

    # TOTAL WATER
Saline_d5_twater_g = Maaslin2(
    input_data = mouse_6_genus, 
    input_metadata = Perc_change_meta_d5_Saline, 
    output = "Saline_d5_twater_g", 
    fixed_effects = c("Abs_Change_Total_Water")) # boring deleted file

Saline_d5_twater_p = Maaslin2(
    input_data = mouse_2_phylum, 
    input_metadata = Perc_change_meta_d5_Saline, 
    output = "Saline_d5_twater_p", 
    fixed_effects = c("Abs_Change_Total_Water")) # boring deleted file

    # FREE WATER
Saline_d5_fwater_g = Maaslin2(
    input_data = mouse_6_genus, 
    input_metadata = Perc_change_meta_d5_Saline, 
    output = "Saline_d5_fwater_g", 
    fixed_effects = c("Abs_Change_Free_Water"))
# signif for this guy: 
# p__Firmicutes.c__Bacilli.o__Lactobacillales.f__Lactobacillaceae.g__
# those that ended up with more at endoint increased their free water retention. 

Saline_d5_fwater_p = Maaslin2(
    input_data = mouse_2_phylum, 
    input_metadata = Perc_change_meta_d5_Saline, 
    output = "Saline_d5_fwater_p", 
    fixed_effects = c("Abs_Change_Free_Water"))

```
But none of those are that interesting because if it increased in all of the mice within a group, there won't be enough variation to detect. I want to see changes in taxonomic composition from D-3 to D5 coupled with the changes in body composition. But that might be too tricky to do within maaslin.  

``` {r}
# What about if I did D5 gut composition in all mice and their changes in each of the categories? Without hormone as an effect? And then tried to color by hormone group. I think that would be best. 

# fat
    # genus level
    grace_play_17_Baseline_microbes_to_fat_change_genus = Maaslin2(
    input_data = mouse_6_genus, 
    input_metadata = Perc_change_meta_d5, 
    output = "D5_microbes_to_fat_change_genus", 
    fixed_effects = c("Abs_Change_Fat")) # none -- deleting
    
    # phylum level
    grace_play_17_Baseline_microbes_to_fat_change_phylum = Maaslin2(
    input_data = mouse_2_phylum, 
    input_metadata = Perc_change_meta_d5, 
    output = "D5_microbes_to_fat_change_phylum", 
    fixed_effects = c("Abs_Change_Fat")) # none -- deleting
    ...
    input_data = mouse_2_phylum, 
    input_metadata = Perc_change_meta_d5, 
    output = "D5_microbes_to_fat_change_phylum_controlledforhormone", 
    fixed_effects = c("Abs_Change_Fat"),
    random_effects = c("Hormone")) # none -- deleting
...
# lean mass
    ...
    input_data = mouse_6_genus, 
    input_metadata = Perc_change_meta_d5, 
    output = "D5_microbes_to_lean_change_genus", 
    fixed_effects = c("Abs_Change_Lean_Mass")) # none -- deleting
    
    # hormone as random effect
    grace_play_17_Baseline_microbes_to_lean_change_genus = Maaslin2(
    input_data = mouse_6_genus, 
    input_metadata = Perc_change_meta_d5, 
    output = "D5_microbes_to_lean_change_genus_controlledforhormone", 
    fixed_effects = c("Abs_Change_Lean_Mass"),
    random_effects = c("Hormone")) # none -- deleting
    
    # hormone as fixed effect without interaction
    grace_play_17_Baseline_microbes_to_lean_change_genus_b = Maaslin2(
    input_data = mouse_6_genus, 
    input_metadata = Perc_change_meta_d5, 
    output = "D5_microbes_to_lean_change_genus_controlledforhormone_b", 
    fixed_effects = c("Abs_Change_Lean_Mass", "Hormone"),
    reference = c("Hormone,Saline")) # none -- deleting
    
    # hormone as fixed effect with interaction
    grace_play_17_Baseline_microbes_to_lean_change_genus_c = Maaslin2(
    input_data = mouse_6_genus, 
    input_metadata = Perc_change_meta_d5, 
    output = "D5_microbes_to_lean_change_genus_controlledforhormone_c", 
    fixed_effects = c("Abs_Change_Lean_Mass", "Hormone", "Abs_Change_Lean_Mass:Hormone"),
    reference = c("Hormone,Saline")) # none -- deleting
    ...
    input_data = mouse_2_phylum, 
    input_metadata = Perc_change_meta_d5, 
    output = "D5_microbes_to_lean_change_phylum_controlledforhormone", 
    fixed_effects = c("Abs_Change_Lean_Mass"),
    random_effects = c("Hormone"))  # none -- deleting
    
```

What about change in body mass from D-3 (or D0) to D5 or D8 vs taxonomy? Like in laura's pic